...
basic <- function(alpha1 = 5, alpha2 = 8, gamma_value = 0.3, sample_size_n1 = 30000, 
                  sample_size_n2 = 30000, bs_round = 3, true_vec=c(1,1), coef_hidden = 1) { 
  ...
  for (i in 1:bs_round) {
    ...
    x11 <- sqrt(5) * u1[, 2]
    x12 <- alpha1 * h1 + u1[, 3]
    y1 <- 1 * x11 + true_vec[2] * x12 + coef_hidden * h1 + sqrt(0.5) * u1[, 1]
    ...
    x21 <- sqrt(0.5) * u2[, 2]
    x22 <- alpha2 * h2 + u2[, 3]
    y2 <- 1 * x21 + true_vec[2] * x22 + coef_hidden * h1 + sqrt(0.5) * u2[, 1]
  ...
  d_S2_temp1 <- sapply(coll_results, function(x) x[[12]]) # rec11
  d_S2_temp2 <- sapply(coll_results, function(x) x[[13]]) # rec12
...
define_alpha1_vec <- seq(5, 5, length.out = 100)
define_alpha2_vec <- seq(8, 8, length.out = 100)
length_alpha <- length(define_alpha1_vec)
define_c_vec <- seq(5, 5, length.out = 100)


plot_result <- function(alpha1_vec = define_alpha1_vec, 
                        alpha2_vec = define_alpha2_vec, 
                        gamma_value_vec = seq(0, 10, length.out = length_alpha), 
                        sample_size_n1_vec = seq(30000, 30000, length.out = length_alpha), 
                        sample_size_n2_vec = seq(30000, 30000, length.out = length_alpha), 
                        bs_round_value = 3, 
                        c_vec = define_c_vec,
                        coef_hidden_vec = seq(5, 5, length.out = length_alpha)){
  ...
  for (i in 1:length_alpha){
    full_result <- basic(alpha1 = alpha1_vec[i], 
                         alpha2 = alpha2_vec[i], 
                         gamma_value = gamma_value_vec[i], 
                         sample_size_n1 = sample_size_n1_vec[i], 
                         sample_size_n2 = sample_size_n2_vec[i], 
                         bs_round = bs_round_value, 
                         true_vec = c(1, c_vec[i]),
                         coef_hidden = coef_hidden_vec[i])
  ...
  # Plot ratio between ratio and l2 error 
  plot_neg_4_x_axis <- seq(1, 100, length.out = 100)
  plot_neg_4_y_axis <- result_l2_error_holder
  dev.new()
  plot(plot_neg_4_x_axis, plot_neg_4_y_axis, type = "p", main = "result_l2_error_holder", xlab = "|coef_hidden_vec - c_vec|", ylab = "l2 error",
       xlim = range(plot_neg_4_x_axis), ylim = range(plot_neg_4_y_axis), cex = 1.5, col = "blue", lwd = 2) 
  
  
  # Plot coef_hidden and the l2 error 
  plot_neg_3_x_axis <- coef_hidden_vec
  plot_neg_3_y_axis <- result_l2_error_holder
  dev.new()
  plot(plot_neg_3_x_axis, plot_neg_3_y_axis, type = "p", main = "result_l2_error_holder", xlab = "coef_hidden", ylab = "l2 error",
       xlim = range(plot_neg_3_x_axis), ylim = range(plot_neg_3_y_axis), cex = 1.5, col = "blue", lwd = 2) 
  
  
  # Plot sample size  and the l2 error 
  plot_neg_2_x_axis <- sample_size_n1_vec
  plot_neg_2_y_axis <- result_l2_error_holder
  dev.new()
  plot(plot_neg_2_x_axis, plot_neg_2_y_axis, type = "p", main = "result_l2_error_holder", xlab = "sample size", ylab = "l2 error",
       xlim = range(plot_neg_2_x_axis), ylim = range(plot_neg_2_y_axis), cex = 1.5, col = "blue", lwd = 2) 
  
  
  
  # Plot gamma value and the l2 error 
  plot_neg_1_x_axis <- gamma_value_vec
  plot_neg_1_y_axis <- result_l2_error_holder
  dev.new()
  plot(plot_neg_1_x_axis, plot_neg_1_y_axis, type = "p", main = "result_l2_error_holder", xlab = "gamma", ylab = "l2 error",
       xlim = range(plot_neg_1_x_axis), ylim = range(plot_neg_1_y_axis), cex = 1.5, col = "blue", lwd = 2) 
  
  
  # Plot c value and the l2 error 
  plot_0_x_axis <- c_vec
  plot_0_y_axis <- result_l2_error_holder
  dev.new()
  plot(plot_0_x_axis, plot_0_y_axis, type = "p", main = "result_l2_error_holder", xlab = "c_vec", ylab = "l2 error",
       xlim = range(plot_0_x_axis), ylim = range(plot_0_y_axis), cex = 1.5, col = "blue", lwd = 2) 
  ...
  dev.new()
  plot(plot_1_x_axis, plot_1_y_matrix[1,], type = "n", xlim = range(plot_1_x_axis), ylim = c(-1,15),
