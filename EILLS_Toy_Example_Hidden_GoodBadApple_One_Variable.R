...
# Changed based on EILLS_Toy_Example_Hidden_GoodBadApple.R. Since I don't know how deal with
# more complicated case, and just want to test out my idea about hiddne variable, I just include 
# one variable X2, which is pointed by a hidden variable H, which points to Y. There is no other 
# variables. We also assume there exists an instrumental variable. Because of this, I need to 
# change the empirical objective. 

library(MASS)
library(combinat)

basic <- function(alpha1 = 5, alpha2 = 8, gamma_value = 10, sample_size_n1 = 3000, 
                  sample_size_n2 = 3000, bs_round = 3, true_vec=c(1,5)) { 
  ...
  for (i in 1:bs_round) {
    ...
    x11 <- sqrt(5) * u1[, 2]
    x12 <- alpha1 * h1 + u1[, 3]
    y1 <- 1 * x11 + true_vec[2] * x12 + 1 * h1 + sqrt(0.5) * u1[, 1]
    ...
    x21 <- sqrt(0.5) * u2[, 2]
    x22 <- alpha2 * h2 + u2[, 3]
    y2 <- 1 * x21 + true_vec[2] * x22 + 1 * h1 + sqrt(0.5) * u2[, 1]
...
define_alpha1_vec <- seq(5, 8, length.out = 100)
define_alpha2_vec <- seq(5, 8, length.out = 100)
...
plot_result <- function(alpha1_vec = define_alpha1_vec, 
                        ...
                        gamma_value_vec = seq(10, 100, length.out = length_alpha), 
