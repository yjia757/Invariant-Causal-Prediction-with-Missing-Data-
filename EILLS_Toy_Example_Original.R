...
basic <- function(alpha1 = 5, alpha2 = 8, gamma_value = 0, sample_size_n1 = 30000, 
                  sample_size_n2 = 30000, bs_round = 3, true_vec = c(1, 0)) { 
  ...
  for (i in 1:bs_round) {
    ...
    rec1 <- cov(x11, sqrt(0.5) * u1[, 1])
    rec2 <- cov(x12, sqrt(0.5) * u1[, 1])
    rec3 <- c(rec1, rec2)
    rec4 <- cov(x21, sqrt(0.5) * u2[, 1])
    rec5 <- cov(x22, sqrt(0.5) * u2[, 1])
    rec6 <- c(rec4, rec5)
    rec7 <- cov(x11, x12)
    rec8 <- cov(x21, x22)
    # This is beta_S^e, where S = {2}, and e = environment 1 
    rec9 <- 0 + solve(mean(x12^2)) * (rec2 + rec7 * 1)
    # This is beta_S^e, where S = {2}, and e = environment 2 
    rec10 <- 0 + solve(mean(x22^2)) * (rec5 + rec8 * 1)
    # This is beta_S^e, where S = {1,2}, and e = environment 1
    rec11 <- c(1,0) + solve(cov(X1[,c(1,2)])) %*% c(rec1, rec2)
    row.names(rec11) <- NULL
    # This is beta_S^e, where S = {1,2}, and e = environment 2
    rec12 <- c(1,0) + solve(cov(X2[,c(1,2)])) %*% c(rec4, rec5)
...
define_alpha1_vec <- seq(8, 5, length.out = 100)
define_alpha2_vec <- seq(3, 1, length.out = 100)
length_alpha <- length(define_alpha1_vec)


plot_result <- function(alpha1_vec = define_alpha1_vec, 
                        alpha2_vec = define_alpha2_vec, 
                        gamma_value_vec = seq(0, 20, length.out = length_alpha), 
